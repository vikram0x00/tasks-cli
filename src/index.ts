...
const getTasks = (status?: TaskStatus): Task[] =>{
	if(status) return tasks.filter((e: Task) => e.status === status);
	return tasks;
}
...
const updateTask = (id: number, status?: TaskStatus, description?: string): Task | void => {
	const task = tasks.find(e => e.id === id);
	if(!task) return console.log("No Tasks Found");
	const otherTasks: Task[] = tasks.filter(e => e.id !== id);
...
const deleteTask = (id: number)=>{
	const task = tasks.find(e => e.id === id);
	if(!task) return console.log("No Task found with the ID", id);
...
const handleMain = () => {
	...
	// List Tasks
		...
		if(!args[1]){
			...
			if(tasks.length === 0) return console.log(styleText("red", "No Tasks Found. Add one to get started"));
			tasks.forEach(task => {
		...
		if(args[1] === "todo" || args[1] === "in-progress" || args[1] === "done"){
			...
			if(tasks.length === 0) return console.log(styleText("red", "No Tasks the requested status were found"));
			tasks.forEach(task => {
				console.log(styleText("green", "ID:"), task.id, styleText("green", "Description:"), task.description, styleText("green", "Created:"), new Date(task.createdAt).toLocaleString().toUpperCase());
			});
		}
